...
import type { DevSoftSkillModel, HardSkillModel, SoftSkillModel } from "@/api/generated/models";
...
export default function SoftSkillList({ profileId }: SoftSkillListProps){
    ...
    const [ selectedSoftSkill, setSelectedSoftSkill ] = useState<DevSoftSkillModel | null>(null);
    ...
    return (
        <div className="w-full flex flex-col gap-4">
            <h2 className='text-3xl flex justify-between'>
                ...
                {softSkillList.length > 1 && (
                    <UpdateSoftSkillModal profileId={profileId} initialData={softSkillList} />
                )}
            </h2>
            <div className="flex flex-col gap-2">
                        ...
                        <Card className="p-0">
                            <Empty>
                                <EmptyHeader>
                                    ...
                                    <EmptyDescription>You haven&apos;t registered any soft yet. Get started by creating your first skill, they are used by our recommendation algorithm</EmptyDescription>
                                </EmptyHeader>
                                <EmptyContent>
                    ...
                    {!isLoading && softSkillList.map((skill) => (
                        <SoftSkillCard key={skill.id} profileId={profileId} openDelete={openDeleteModal} openUpdate={openUpdateModal} devSoftSkill={skill} />
                    ))}
            </div>
        </div>
    );
