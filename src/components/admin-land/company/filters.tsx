import { useEnumOperationalSegments } from "@/api/generated/enums/enums";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useIndexCompanyParams } from "@/hooks/filters/use-index-company-params";
import { BrushCleaning, Filter, Search } from "lucide-react";
import { useTranslation } from "react-i18next";

export function Filters() {
    const { t } = useTranslation();

    const {
        search,
        operational_segment,
        setFilterParams,
    } = useIndexCompanyParams();

    const clearFilters = () => {
        setFilterParams({
            search: "",
            operational_segment: "",
        });
    };

    const { data: segments } = useEnumOperationalSegments();
    const segmentsList = segments?.data || [];

    return (
        <Card className="p-4 mt-2 flex flex-row gap-2 mb-2">
            <div className="relative flex flex-1">
                <Search className="size-4 absolute left-2 top-1/2 -translate-y-1/2 text-muted-foreground" />
                <span className="sr-only">User</span>
                <Input
                    value={search}
                    onChange={(e) => setFilterParams({ search: e.target.value })}
                    type="text"
                    placeholder={t("placeholder.company_search")}
                    className="peer pl-9 bg-background"
                />
            </div>
            <Popover>
                <PopoverTrigger asChild>
                    <Button variant={"default"}><Filter className="size-4" /></Button>
                </PopoverTrigger>
                <PopoverContent align="end" className="w-80 flex flex-col gap-4">
                    <p className="font-medium">{t("general.filters")}</p>
                    <div className="flex flex-col gap-2">
                        <Label>{t("input.operational_segment")}</Label>
                        <Select onValueChange={(value) => setFilterParams({ operational_segment: value === "all" ? "" : value })}>
                            <SelectTrigger className="w-full">
                                <SelectValue placeholder={t("placeholder.filter_operational_segment")} />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectItem value="all">{t("filters.all")}</SelectItem>
                                {segmentsList.map((segment) => (
                                    <SelectItem key={segment.value} value={segment.value}>
                                        {t(segment.i18nKey)}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                </PopoverContent>
            </Popover>
            <Button variant={"secondary"} onClick={clearFilters}>
                <BrushCleaning className="size-4" /> {t("general.clear")}
            </Button>
        </Card>
    );
}
