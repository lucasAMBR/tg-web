...
export default function AddressForm(){
    ...
    useEffect(() => {
        ...
        async function fetchAddress() {
            const res = await fetch(`${env.VITE_AWESOME_API_URL}/${cep}?token=${env.VITE_AWESOME_API_KEY}`);
    ...
    return(
        <form onSubmit={form.handleSubmit(createAddress)} className="w-full max-w-[700px] flex flex-col gap-4">
            <div className="flex gap-2">
                <Controller 
                    ...
                    render={({ field, fieldState}) => (
                        <Field className="flex-1">
                            <FieldLabel>CEP <Required /></FieldLabel>
                            <Input
                                {...field}  
                ...
                <Field className="flex-3">
                    ...
                    <Input value={address.street} readOnly placeholder="Street" className="cursor-not-allowed" disabled/>
                </Field>
            </div>
            <div className="flex gap-2">
                <Field className="flex-4">
                    <FieldLabel>District</FieldLabel>
                    <Input value={address.district} readOnly placeholder="District" className="cursor-not-allowed" disabled/>
                </Field>
                <Field className="flex-1">
                    <FieldLabel>City</FieldLabel>
                    <Input value={address.city} readOnly placeholder="City" className="cursor-not-allowed" disabled/>
                </Field>
                <Field className="flex-1">
                    <FieldLabel>State</FieldLabel>
                    <Input value={address.state} readOnly placeholder="State" className="cursor-not-allowed" disabled/>
                </Field>
            </div>
            <div className="flex gap-2">
                <Controller 
                    ...
                    render={({ field, fieldState}) => (
                        <Field className="flex-1">
                            <FieldLabel>Number <Required /></FieldLabel>
                            <Input
                                {...field}  
                                aria-invalid={fieldState.invalid}
                                id="number"
